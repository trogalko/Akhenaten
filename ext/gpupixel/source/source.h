...
public:
    Source();
    virtual ~Source();
    virtual SourcePtr addTarget(std::shared_ptr<Target> target);
    virtual SourcePtr addTarget(std::shared_ptr<Target> target,
        int texIdx);
    ...
    virtual std::map<std::shared_ptr<Target>, int> &getTargets() { return _targets; };

    virtual void setFramebuffer(std::shared_ptr<Framebuffer> fb, RotationMode outputRotation = RotationMode::NoRotation);
    ...
    virtual unsigned char *captureAProcessedFrameData(std::shared_ptr<Filter> upToFilter, int width = 0, int height = 0);
