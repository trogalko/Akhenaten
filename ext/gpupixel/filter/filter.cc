...
FilterPtr Filter::create(const std::string &filterClassName) {
...
FilterPtr Filter::createWithShaderString(const std::string &vertexShaderSource, const std::string &fragmentShaderSource) {
    auto filter = FilterPtr(new Filter());
...
FilterPtr Filter::createWithFragmentShaderString(const std::string &fragmentShaderSource) {
    auto filter = FilterPtr(new Filter());
...
bool Filter::initWithFragmentShaderString(
    ...
    int inputNumber /* = 1*/) {
    return initWithShaderString(_getVertexShaderString(inputNumber), fragmentShaderSource, inputNumber);
}
...
bool Filter::proceed(bool bUpdateTargets /* = true*/, int64_t frametime /* = 0*/) {
...

    {
        ...
        for (std::map<int, InputFrameBufferInfo>::const_iterator it = _inputFramebuffers.begin(); it != _inputFramebuffers.end(); ++it) {
            ...
            _filterProgram->setUniformValue(texIdx == 0 ? "inputImageTexture" : Util::str_format("inputImageTexture%d", texIdx), texIdx);
            // texcoord attribute
            GLuint filterTexCoordAttribute = _filterProgram->getAttribLocation(texIdx == 0 ? "inputTextureCoordinate" : Util::str_format("inputTextureCoordinate%d", texIdx));
...
void Filter::update(int64_t frameTime) {
    ...
    if (GPUPixelContext::getInstance()->isCapturingFrame && this == GPUPixelContext::getInstance()->captureUpToFilter.get()) {
        int captureWidth = GPUPixelContext::getInstance()->captureWidth;
        int captureHeight = GPUPixelContext::getInstance()->captureHeight;

        if (!_framebuffer || (_framebuffer->getWidth() != captureWidth || _framebuffer->getHeight() != captureHeight)) {
            _framebuffer = GPUPixelContext::getInstance()->getFramebufferCache()->fetchFramebuffer(captureWidth, captureHeight);
        }
        ...
        // todo(Jeayo)
        std::shared_ptr<Framebuffer> firstInputFramebuffer = _inputFramebuffers.begin()->second.frameBuffer;
        ...
        if (!_framebuffer || (_framebuffer->getWidth() != rotatedFramebufferWidth ||
            _framebuffer->getHeight() != rotatedFramebufferHeight)) {
            _framebuffer = GPUPixelContext::getInstance()->getFramebufferCache()->fetchFramebuffer(rotatedFramebufferWidth, rotatedFramebufferHeight);
        }
