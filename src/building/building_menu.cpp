...
void building_menu_ctrl_t::register_callbacks() {
    events::subscribe([] (event_temple_complex_updated ev) {
        g_building_menu_ctrl.update_temple_complexes();
    });

    events::subscribe([] (event_building_menu_update ev) {
        g_building_menu_ctrl.update(ev.stage);
    });

    events::subscribe([] (event_religion_god_status_update ev) {
        g_building_menu_ctrl.update_gods_available(ev.god, ev.status);
    });
}
...
bool building_menu_ctrl_t::has_changed() {
    if (changed) {
        changed = false;
        return true;
    }
